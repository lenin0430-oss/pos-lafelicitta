...
export default function StockPage() {
  ...
  function abrirEdicion(item: StockItem) {
    setEditandoId(item.id); setFormNombre(item.nombre); setFormUnidad(item.unidad)
    setFormMinimo(String(item.stock_minimo)); setFormCategoria(item.categoria)
    setFormPrecio(String(item.precio_ultimo)); setFormStockInicial(String(item.stock_actual))
    setMostrarFormNuevo(true); window.scrollTo({ top: 0, behavior: 'smooth' })
  }
  ...
  async function crearInsumo() {
    ...
    if (editandoId) {
      ...
      if (error) { mostrarMensaje('Error: ' + error.message, 'err') } else { mostrarMensaje('Insumo actualizado ✓', 'ok') }
    } else {
      const { error } = await supabase.from('stock_insumos').insert({ ...datos, empresa_id: empresaId })
      if (error) { mostrarMensaje('Error: ' + error.message, 'err') } else { mostrarMensaje('Insumo creado ✓', 'ok') }
    }
  ...
  async function registrarMerma() {
    ...
    mostrarMensaje(`${TIPO_CONFIG[mermaTipo].label} registrado: -${cantidad} ${item.unidad} de ${item.nombre}`, 'ok')
  ...
  const historialUnificado: EntradaHistorial[] = [
    ...compras.map(c => ({ _tipo: 'compra' as const, fecha: c.fecha, data: c })),
    ...movimientos.map(m => ({ _tipo: 'movimiento' as const, fecha: m.fecha, data: m })),
  ].sort((a, b) => b.fecha.localeCompare(a.fecha))
  const historialFiltrado = historialUnificado.filter(e => {
    if (historialFiltro === 'compras') return e._tipo === 'compra'
    if (historialFiltro === 'movimientos') return e._tipo === 'movimiento'
    return true
  })
        ...
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '20px 14px' }}>
          <div style={{ marginBottom: 16 }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: 13, marginTop: 4 }}>
              ...
              {perdidaMermas > 0 && <> · Pérdida mermas: <strong style={{ color: 'var(--red)' }}>{fmt(perdidaMermas)}</strong></>}
            </p>
          </div>
          ...
          <div style={{ display: 'flex', gap: 0, marginBottom: 20, borderBottom: '1px solid var(--border)' }}>
            {([['stock','📦 Stock'],['comprar','🛍 Compra del día'],['mermas','⚠️ Mermas'],['historial','📋 Historial']] as const).map(([t, label]) => (
              <button key={t} onClick={() => setTab(t)} style={{ padding: '10px 16px', border: 'none', background: 'transparent', color: tab === t ? 'var(--gold)' : 'var(--muted)', fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font)', fontSize: 13, borderBottom: tab === t ? '2px solid var(--gold)' : '2px solid transparent', marginBottom: -1, whiteSpace: 'nowrap' }}>{label}</button>
            ))}
          </div>
          ...
          {tab === 'stock' && (
            <div>
              ...
              {mostrarFormNuevo && (
                <div style={{ background: 'var(--surface)', border: '1px solid var(--gold)', borderRadius: 12, padding: 16, marginBottom: 20 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                    ...
                    <div><label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>Unidad de medida</label><select value={formUnidad} onChange={e => setFormUnidad(e.target.value)} style={sel}>{UNIDADES.map(u => <option key={u}>{u}</option>)}</select></div>
                    <div><label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>Precio de compra ($)</label><input type="number" value={formPrecio} onChange={e => setFormPrecio(e.target.value)} placeholder="Ej: 5000" style={inp} /></div>
                    <div><label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>Stock mínimo (alerta)</label><input type="number" value={formMinimo} onChange={e => setFormMinimo(e.target.value)} placeholder="Ej: 1" style={inp} /></div>
                    <div><label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>Stock inicial actual</label><input type="number" value={formStockInicial} onChange={e => setFormStockInicial(e.target.value)} placeholder="0" style={inp} /></div>
                  </div>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={crearInsumo} disabled={guardando} style={{ flex: 1, padding: '10px', borderRadius: 8, border: 'none', background: 'var(--gold)', color: '#000', fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font)' }}>{guardando ? '...' : editandoId ? '✓ Guardar cambios' : '✓ Crear insumo'}</button>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 16 }}>
                {[{ label: '🟢 OK', val: ok.length, color: 'var(--green)' }, { label: '🟡 Bajo', val: bajos.length, color: 'var(--gold)' }, { label: '🔴 Agotado', val: agotados.length, color: 'var(--red)' }].map(s => (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {itemsFiltrados.map(item => {
                  ...
                  return (
                    <div key={item.id} style={{ background: 'var(--surface)', border: `1px solid ${item.stock_actual <= 0 ? 'var(--red)' : item.stock_actual <= item.stock_minimo ? 'var(--gold)' : 'var(--border)'}`, borderRadius: 12, padding: '12px 14px' }}>
                      ...
                      <div style={{ height: 6, background: 'var(--surface2)', borderRadius: 3, overflow: 'hidden' }}><div style={{ height: '100%', width: `${pct}%`, background: color, borderRadius: 3, transition: 'width .3s' }} /></div>
                ...
                {itemsFiltrados.length === 0 && <div style={{ textAlign: 'center', padding: 50, color: 'var(--muted)', fontSize: 13 }}>{items.length === 0 ? 'No hay insumos — agrega el primero con "+ Nuevo insumo"' : 'No se encontraron resultados'}</div>}
              </div>
            </div>
          )}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 10 }}>
                  <div><label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>¿Qué compraste?</label>
                    <select value={compraItem} onChange={e => setCompraItem(e.target.value)} style={sel}>
                      ...
                      {items.map(i => <option key={i.id} value={i.id}>{i.nombre} (stock actual: {i.stock_actual} {i.unidad})</option>)}
                    </select>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                    <div><label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>Cantidad comprada {compraItem && `(${items.find(i=>i.id===compraItem)?.unidad})`}</label><input type="number" value={compraCantidad} onChange={e => setCompraCantidad(e.target.value)} placeholder="Ej: 5" style={inp} min={0} step={0.1} /></div>
                    <div><label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>Precio total pagado ($)</label><input type="number" value={compraPrecio} onChange={e => setCompraPrecio(e.target.value)} placeholder="Ej: 8500" style={inp} /></div>
                  </div>
                  {compraCantidad && compraPrecio && parseFloat(compraCantidad) > 0 && (
                    <div style={{ padding: '8px 12px', background: 'rgba(212,168,67,.1)', border: '1px solid var(--gold)', borderRadius: 8, fontSize: 13 }}>
                      ...
                      {compraItem && (() => { const item = items.find(i => i.id === compraItem); if (!item || item.precio_ultimo === 0) return null; const nuevo = parseFloat(compraPrecio) / parseFloat(compraCantidad); const diff = ((nuevo - item.precio_ultimo) / item.precio_ultimo) * 100; if (Math.abs(diff) > 5) return <span style={{ marginLeft: 10, color: diff > 0 ? 'var(--red)' : 'var(--green)', fontWeight: 700 }}>{diff > 0 ? `▲ +${Math.round(diff)}% vs última compra` : `▼ ${Math.round(diff)}% vs última compra`}</span> })()}
                    </div>
                  )}
                  <div><label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>Nota (opcional)</label><input value={compraNota} onChange={e => setCompraNota(e.target.value)} placeholder="Ej: Feria, Mayorista, precio especial..." style={inp} /></div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {comprasHoy.map(c => (
                      <div key={c.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div><div style={{ fontWeight: 600, fontSize: 13 }}>{c.insumo_nombre}</div><div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>{c.cantidad} {items.find(i => i.id === c.insumo_id)?.unidad ?? 'und'} · {fmt(c.precio_unitario)}/u{c.nota && ` · ${c.nota}`}</div></div>
                ...
                <div style={{ display: 'grid', gap: 10 }}>
                  <div><label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>Insumo</label>
                    <select value={mermaItem} onChange={e => setMermaItem(e.target.value)} style={sel}>
                      <option value="">— Selecciona insumo —</option>
                      {items.filter(i => i.stock_actual > 0).map(i => <option key={i.id} value={i.id}>{i.nombre} (disponible: {i.stock_actual} {i.unidad})</option>)}
                    </select>
                  </div>
                  <div><label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>Cantidad {mermaItem && `(${items.find(i=>i.id===mermaItem)?.unidad})`}</label><input type="number" value={mermaCantidad} onChange={e => setMermaCantidad(e.target.value)} placeholder="Ej: 0.5" style={inp} min={0} step={0.01} /></div>
                  ...
                  <div><label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>Motivo</label><input value={mermaMotivo} onChange={e => setMermaMotivo(e.target.value)} placeholder="Ej: Se venció, se cayó, consumo del personal..." style={inp} /></div>
          ...
          {tab === 'historial' && (
            <div>
              <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
                {([['todos','Todos'],['compras','🛍 Compras'],['movimientos','⚠️ Mermas / Ajustes']] as const).map(([v, label]) => (
                  <button key={v} onClick={() => setHistorialFiltro(v)} style={{ padding: '6px 14px', borderRadius: 20, border: `1px solid ${historialFiltro === v ? 'var(--gold)' : 'var(--border)'}`, background: historialFiltro === v ? 'var(--gold)' : 'transparent', color: historialFiltro === v ? '#000' : 'var(--muted)', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font)' }}>{label}</button>
                ))}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {historialFiltrado.map((entrada, idx) => {
                  if (entrada._tipo === 'compra') {
                    ...
                    return (
                      <div key={`c-${c.id}-${idx}`} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--green)', background: 'rgba(76,175,125,.12)', border: '1px solid var(--green)', borderRadius: 4, padding: '1px 6px' }}>🛍 COMPRA</span>
                            <span style={{ fontWeight: 600, fontSize: 13 }}>{c.insumo_nombre}</span>
                          </div>
                          <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 3 }}>+{c.cantidad} {unidad} · {fmt(c.precio_unitario)}/{unidad} · {new Date(c.fecha).toLocaleDateString('es-CL', { day: '2-digit', month: '2-digit', year: '2-digit' })}{c.nota && ` · ${c.nota}`}</div>
                        </div>
                    ...
                    return (
                      <div key={`m-${m.id}-${idx}`} style={{ background: 'var(--surface)', border: `1px solid ${m.tipo === 'merma' ? 'rgba(220,50,50,.3)' : 'var(--border)'}`, borderRadius: 10, padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            <span style={{ fontSize: 10, fontWeight: 700, color: cfg.color, background: m.tipo === 'merma' ? 'rgba(220,50,50,.1)' : 'rgba(212,168,67,.1)', border: `1px solid ${cfg.color}`, borderRadius: 4, padding: '1px 6px' }}>{cfg.label.toUpperCase()}</span>
                            <span style={{ fontWeight: 600, fontSize: 13 }}>{m.insumo_nombre}</span>
                          </div>
                          <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 3 }}>-{m.cantidad} {item?.unidad ?? 'und'} · {m.motivo} · {new Date(m.fecha).toLocaleDateString('es-CL', { day: '2-digit', month: '2-digit', year: '2-digit' })}</div>
                        </div>
