...
export default function CosteoPage() {
  ...
  async function cargar() {
    ...
    const catalogoPromise = empresaId ? cargarDatosEmpresa(empresaId) : Promise.resolve({ menu: MENU, categorias: CATEGORIAS, mesas: [], metodosPago: [] })
    const [{ data: ins }, { data: rec }, catalogo] = await Promise.all([
      empresaId ? supabase.from('stock_insumos').select('id, nombre, precio_ultimo, stock_actual, unidad, categoria').eq('empresa_id', empresaId).order('nombre') : Promise.resolve({ data: [] }),
  ...
  const margenReal = (costo: number, precioMenu: number) => precioMenu > 0 ? ((precioMenu - costo) / precioMenu) * 100 : null
  ...
  function agregarIngrediente() {
    ...
    if (!ins) return
    const costo = ins.precio_ultimo * parseFloat(cantSel)
  ...
  async function guardarReceta() {
    ...
    if (editandoId) { await supabase.from('recetas').update(datos).eq('empresa_id', empresaId).eq('id', editandoId) }
    else { await supabase.from('recetas').insert({ ...datos, empresa_id: empresaId }) }
  ...
  function editarReceta(r: Receta) { setEditandoId(r.id); setFormNombre(r.producto_nombre); setFormIngredientes(r.ingredientes); setTab('recetas') }
      ...
      <main style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', paddingBottom: 60 }}>
        ...
        <div style={{ maxWidth: 780, margin: '0 auto', padding: '20px 14px' }}>
          <div style={{ marginBottom: 20 }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: 13, marginTop: 4 }}>{recetas.length} platos costeados · {insumos.length} insumos · {sinReceta.length} sin receta</p>
          </div>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 20, borderBottom: '1px solid var(--border)' }}>
            {([['costeo','📊 Costeo'],['recetas','📋 Recetas']] as const).map(([t, label]) => (
              <button key={t} onClick={() => setTab(t)} style={{ padding: '10px 18px', border: 'none', background: 'transparent', color: tab === t ? 'var(--gold)' : 'var(--muted)', fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font)', fontSize: 14, borderBottom: tab === t ? '2px solid var(--gold)' : '2px solid transparent', marginBottom: -1 }}>{label}</button>
            ))}
            <a href="/stock" style={{ padding: '10px 18px', color: 'var(--muted)', fontWeight: 700, fontFamily: 'var(--font)', fontSize: 14, textDecoration: 'none', marginLeft: 'auto' }}>🧂 Insumos → Stock</a>
          </div>
          {tab === 'costeo' && (
            <div>
              ...
              {conReceta.length > 0 && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 20 }}>
                  {[{ label: 'Con receta', val: conReceta.length, color: 'var(--green)' }, { label: 'Sin receta', val: sinReceta.length, color: 'var(--red)' }, { label: 'Margen prom.', val: (() => { const ms = conReceta.map(p => margenReal(p.receta!.costo_total, p.precio)).filter(m => m !== null) as number[]; return ms.length > 0 ? Math.round(ms.reduce((a,b)=>a+b,0)/ms.length) + '%' : '-' })(), color: 'var(--gold)' }].map(s => (
              ...
              {conReceta.length > 0 && (
                <div style={{ marginBottom: 24 }}>
                  ...
                  {conReceta.map(p => {
                    const costo = p.receta!.costo_total; const sugerido = precioSugerido(costo); const mReal = margenReal(costo, p.precio)
                    ...
                    return (
                      <div key={p.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 14, marginBottom: 10 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                          <div><div style={{ fontWeight: 700, fontSize: 15 }}>{p.nombre}</div><div style={{ fontSize: 11, color: 'var(--muted)' }}>{p.categoria}</div></div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
                          {[{ label: 'Costo', val: fmt(costo), color: 'var(--red)' }, { label: 'Precio actual', val: fmt(p.precio), color: 'var(--text)' }, { label: `Sugerido (${margen}%)`, val: fmt(sugerido), color: 'var(--gold)' }, { label: 'Margen real', val: mReal !== null ? Math.round(mReal) + '%' : '-', color: mColor }].map(col => (
                            <div key={col.label} style={{ background: 'var(--bg)', borderRadius: 8, padding: '8px 10px', textAlign: 'center' }}>
                              <div style={{ fontSize: 10, color: 'var(--muted)', marginBottom: 3 }}>{col.label}</div>
                              <div style={{ fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 14, color: col.color }}>{col.val}</div>
                            </div>
                          ))}
                        </div>
                        {mReal !== null && mReal < 50 && <div style={{ marginTop: 10, padding: '6px 10px', background: 'rgba(220,50,50,.1)', border: '1px solid var(--red)', borderRadius: 7, fontSize: 12, color: 'var(--red)' }}>⚠️ Margen bajo</div>}
                        <details style={{ marginTop: 10 }}><summary style={{ fontSize: 12, color: 'var(--muted)', cursor: 'pointer' }}>Ver ingredientes ({p.receta!.ingredientes.length})</summary>
                          {p.receta!.ingredientes.map((ing, i) => (<div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, padding: '4px 0', borderBottom: '1px solid var(--border)' }}><span style={{ color: 'var(--muted)' }}>{ing.nombre} · {ing.cantidad} {ing.unidad}</span><span style={{ fontFamily: 'var(--mono)' }}>{fmt(ing.costo)}</span></div>))}
                        </details>
                      </div>
                    )
                  })}
              ...
              {sinReceta.length > 0 && (
                <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 8 }}>
                    {sinReceta.map(p => (<button key={p.id} onClick={() => { setFormNombre(p.nombre); setTab('recetas') }} style={{ background: 'var(--surface)', border: '1px dashed var(--border)', borderRadius: 10, padding: '12px 14px', textAlign: 'left', cursor: 'pointer', fontFamily: 'var(--font)', color: 'var(--text)' }}><div style={{ fontSize: 14, fontWeight: 600 }}>{p.nombre}</div><div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>{p.categoria} · {fmt(p.precio)}</div><div style={{ fontSize: 11, color: 'var(--gold)', marginTop: 6 }}>+ Crear receta →</div></button>))}
                  </div>
                </div>
              )}
            </div>
            ...
            <div>
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 18, marginBottom: 24 }}>
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  <datalist id="menu-productos">{menu.map(p => <option key={p.id} value={p.nombre} />)}</datalist>
                </div>
                <div style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 100px auto', gap: 8 }}>
                    ...
                    <input type="number" value={cantSel} onChange={e => setCantSel(e.target.value)} placeholder={insumoActual ? insumoActual.unidad : 'Cant.'} style={inp} min={0} step={0.01} />
                  ...
                  {insumoActual && cantSel && parseFloat(cantSel) > 0 && <div style={{ marginTop: 8, padding: '6px 10px', background: 'rgba(212,168,67,.1)', border: '1px solid var(--gold)', borderRadius: 8, fontSize: 12 }}>💡 Costo: <strong style={{ color: 'var(--gold)' }}>{fmt(insumoActual.precio_ultimo * parseFloat(cantSel))}</strong></div>}
                </div>
                {formIngredientes.length > 0 && (
                  <div style={{ background: 'var(--bg)', borderRadius: 10, padding: 12, marginBottom: 14 }}>
                    {formIngredientes.map((ing, i) => (<div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 0', borderBottom: '1px solid var(--border)', fontSize: 13 }}><span>{ing.nombre} · {ing.cantidad} {ing.unidad}</span><div style={{ display: 'flex', gap: 10 }}><span style={{ fontFamily: 'var(--mono)', color: 'var(--gold)' }}>{fmt(ing.costo)}</span><button onClick={() => setFormIngredientes(prev => prev.filter((_, j) => j !== i))} style={{ background: 'transparent', border: 'none', color: 'var(--red)', cursor: 'pointer' }}>✕</button></div></div>))}
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 10, fontWeight: 700 }}><span>Costo total:</span><span style={{ fontFamily: 'var(--mono)', color: 'var(--red)' }}>{fmt(formIngredientes.reduce((s, i) => s + i.costo, 0))}</span></div>
                  </div>
                )}
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={guardarReceta} disabled={guardando} style={{ flex: 1, padding: '10px', borderRadius: 10, border: 'none', background: 'var(--gold)', color: '#000', fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font)' }}>{guardando ? '...' : editandoId ? '✓ Actualizar receta' : '✓ Guardar receta'}</button>
                ...
                {insumos.length === 0 && <div style={{ marginTop: 12, padding: '8px 12px', background: 'rgba(212,168,67,.1)', border: '1px solid var(--gold)', borderRadius: 8, fontSize: 12, color: 'var(--gold)' }}>⚠️ No hay insumos — agrégalos en <a href="/stock" style={{ color: 'var(--gold)', fontWeight: 700 }}>Stock</a></div>}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {recetas.map(r => (<div key={r.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 14 }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><div><div style={{ fontWeight: 700 }}>{r.producto_nombre}</div><div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>{r.ingredientes.length} ingredientes</div></div><div style={{ display: 'flex', gap: 8 }}><span style={{ fontFamily: 'var(--mono)', fontWeight: 700, color: 'var(--red)' }}>{fmt(r.costo_total)}</span><button onClick={() => editarReceta(r)} style={{ background: 'transparent', border: '1px solid var(--border)', borderRadius: 6, color: 'var(--muted)', padding: '4px 8px', cursor: 'pointer', fontSize: 12, fontFamily: 'var(--font)' }}>✏️</button><button onClick={() => eliminarReceta(r.id)} style={{ background: 'transparent', border: '1px solid var(--red)', borderRadius: 6, color: 'var(--red)', padding: '4px 8px', cursor: 'pointer', fontSize: 12, fontFamily: 'var(--font)' }}>🗑</button></div></div></div>))}
