...
export default function PropinasPage() {
  ...
  async function registrarPropina() {
    ...
    if (!monto || parseInt(monto) <= 0) { mostrarMensaje('Ingresa un monto válido', 'err'); return }
  ...
  const inp: React.CSSProperties = { background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text)', padding: '10px 12px', fontFamily: 'var(--font)', fontSize: 14, outline: 'none', width: '100%' }
    ...
    <AuthGuard>
      ...
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '20px 16px', fontFamily: 'var(--font)' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          {mensaje && (
            <div style={{ marginTop: 10, padding: '8px 12px', borderRadius: 8, background: mensaje.tipo === 'ok' ? 'rgba(76,175,125,0.1)' : 'rgba(220,53,69,0.1)', border: '1px solid ' + (mensaje.tipo === 'ok' ? 'var(--green)' : 'var(--red)'), color: mensaje.tipo === 'ok' ? 'var(--green)' : 'var(--red)', fontSize: 13 }}>
              {mensaje.txt}
            </div>
          )}
        </div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 20, marginBottom: 24 }}>
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 5 }}>Método</label>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' as const }}>
              {METODOS.map(m => (
                <button key={m} onClick={() => setMetodo(m)}
                  style={{ padding: '7px 14px', borderRadius: 20, border: '1px solid ' + (m === metodo ? 'var(--gold)' : 'var(--border)'), background: m === metodo ? 'var(--gold)' : 'transparent', color: m === metodo ? '#000' : 'var(--muted)', fontSize: 13, cursor: 'pointer', fontFamily: 'var(--font)' }}>
                  {m}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <input value={notas} onChange={e => setNotas(e.target.value)} placeholder="Mesa 3, pedido especial..." style={inp} />
          </div>

          <button onClick={registrarPropina} disabled={guardando}
            style={{ width: '100%', padding: 14, borderRadius: 10, border: 'none', background: 'var(--gold)', color: '#000', fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font)' }}>
        ...
        {porEmpleado.length > 0 && (
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 20, marginBottom: 24 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              <div style={{ fontSize: 12, color: 'var(--muted)', letterSpacing: 1, textTransform: 'uppercase' as const }}>Resumen del día</div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 20 }}>
          ...
          {cargando ? (
            ...
            <div style={{ color: 'var(--muted)', fontSize: 13, textAlign: 'center', padding: 20 }}>Sin propinas registradas este día</div>
          ) : propinas.map(p => (
            <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
              <div>
                <div style={{ fontSize: 14, fontWeight: 600 }}>{p.empleado}</div>
                <div style={{ fontSize: 12, color: 'var(--muted)' }}>{p.metodo_pago} · {new Date(p.created_at).toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit' })}{p.notas ? ' · ' + p.notas : ''}</div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{ fontFamily: 'var(--mono)', color: 'var(--gold)', fontWeight: 700 }}>{fmt(p.monto)}</span>
                {isAdmin && (
                  <button onClick={() => eliminarPropina(p.id)}
