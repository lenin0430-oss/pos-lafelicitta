...
interface Insumo { id: string; nombre: string; precio_ultimo: number; stock_actual: number; unidad: string }
...
export default function RecetasPage() {
  ...
  async function cargarInsumos() {
    ...
    const { data } = await supabase.from('stock_insumos').select('id,nombre,precio_ultimo,stock_actual,unidad').eq('empresa_id', empresaId).order('nombre')
  ...
  function agregarIngrediente() {
    ...
    if (!ins) return
    setIngredientes([...ingredientes,{insumoId:ins.id,nombre:ins.nombre,cantidad:parseFloat(cantSel),unidad:ins.unidad,costo:ins.precio_ultimo*parseFloat(cantSel)}])
    setInsumoSel(''); setCantSel('')
  }

  async function guardar() {
    if (!nombre.trim()) { mostrarMensaje('Ingresa el nombre','err'); return }
    if (!ingredientes.length) { mostrarMensaje('Agrega al menos un ingrediente','err'); return }
    const empresaId = await getEmpresaIdActual()
    if (!empresaId) { mostrarMensaje('No hay empresa activa','err'); return }
    const datos = {producto_nombre:nombre.trim(),ingredientes,costo_total:ingredientes.reduce((s,i)=>s+i.costo,0)}
      ...
      <div style={{maxWidth:640,margin:'0 auto',padding:'24px 16px'}}>
        ...
        <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,padding:16,marginBottom:20}}>
          <div style={{marginBottom:10}}>
            ...
            <input style={inp} value={nombre} onChange={e=>setNombre(e.target.value)} placeholder="Ej: 1/4 Pollo a la Brasa Solo" />
          </div>
            ...
            <div key={r.id} style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:10,padding:'12px 14px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                ...
                <div style={{display:'flex',gap:8,alignItems:'center'}}>
                  ...
                  <button onClick={()=>{setEditandoId(r.id);setNombre(r.producto_nombre);setIngredientes(r.ingredientes)}} style={{background:'transparent',border:'1px solid var(--border)',borderRadius:6,padding:'3px 8px',color:'var(--muted)',cursor:'pointer',fontSize:12}}>Editar</button>
      ...
      <main style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', paddingBottom: 40 }}>
        ...
        <div style={{ maxWidth: 640, margin: '0 auto', padding: '24px 16px' }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 20 }}>
            <h1 style={{ fontSize: 22, fontWeight: 700, margin: '0 0 8px', color: 'var(--gold)' }}>📋 Recetas</h1>
            <p style={{ color: 'var(--muted)', fontSize: 14, margin: '0 0 16px' }}>
              Las recetas ahora se crean y editan desde la pantalla de Costeo para evitar duplicar insumos.
            </p>
            <Link href="/costeo" style={{ display: 'block', textAlign: 'center', padding: '12px 14px', borderRadius: 10, background: 'var(--gold)', color: '#000', fontWeight: 800, textDecoration: 'none' }}>
              Ir a Costeo y Recetas
            </Link>
          </div>
        </div>
      </main>
