...
export default function CajaPage() {
      ...
      <div style={{ padding: 12, display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 8, overflowY: 'auto' }}>
        {productosFiltrados.map(p => (
          <button key={p.id} onClick={() => agregarProducto(p)}
            ...
            <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 6, lineHeight: 1.3 }}>{p.nombre}</div>
            <div style={{ fontFamily: 'var(--mono)', fontSize: 13, color: 'var(--gold)', fontWeight: 700 }}>{fmt(p.precio)}</div>
          </button>
        ))}
      </div>
    </div>
  ...
  return (
    <>
      ...
      <style>{`
        @media print { body { background: white !important; } #ticket-print { display: block !important; } .no-print { display: none !important; } header { display: none !important; } footer { display: none !important; } .mobile-layout { display: none !important; } .desktop-layout { display: none !important; } }
